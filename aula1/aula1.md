...
Para humanos que conhecem e dominam a linguagem natural, uma ordem como "faça um miojo" é simples e sem ambiguidades, mas num contexto computacional (programação) isso é inviável, é preciso ser mais claro e direto ao dar instruções a serem executadas por um computador. 

Um computador não compreende linguagem natural, não é possível criar um programa dando instruções em português ou em outro idioma. Por isso, usamos linguagens de programação, são linguagens menos complexas, mais próximas da máquina do que do homem, aqui será o PYTHON. 
...
### Markdown:
As células Markdown são aquelas que você pode editar, e servem como explicação do seu notebook. Para criar um markdown, basta inserir uma célula e trocar de code para markdown. Células markdown possuem sintaxe própria, dá pra usar marcadores para formatar o texto.
